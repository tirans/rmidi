...
def main():
    ...
    success = run_command(
        "python -m pip show pytest", "1. Checking if pytest is installed"
    )
    ...
    success = run_command(
        "pytest --collect-only -q", "2. Checking pytest test discovery"
    )
    ...
    test_dirs = ["tests/unit/server", "tests/unit/r2midi_client"]
