...
def run_pytest_checks():
    ...
    code, stdout, stderr = run_command(
        "pytest --collect-only -v 2>&1 | grep -E '(ImportError|ModuleNotFoundError)' || echo 'No import errors found'"
    )
    print(stdout.strip())
    ...
    print("-" * 40)
    code, stdout, stderr = run_command(
        "pytest tests/unit/server/test_models.py::TestModels::test_device_model -v"
    )
...
def run_full_test_suite():
    ...
    code, _, _ = run_command(
        "pytest -v --cov=server --cov=r2midi_client --cov-report=term-missing",
        capture=False,
    )
