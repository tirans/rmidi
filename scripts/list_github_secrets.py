    ...
    for secret in used_secrets:
        ...
        if value:
            ...
            if len(value) > 100 and (
                "P12" in secret or "CERTIFICATE" in secret or "KEY" in secret
            ):
...
def print_github_secrets(secrets):
    ...
    if not found_used_secrets:
        print(
            f"{Colors.FAIL}No used secrets found in environment variables.{Colors.ENDC}"
        )
    ...
    if not found_unused_secrets:
        print(
            f"{Colors.FAIL}No unused secrets found in environment variables.{Colors.ENDC}"
        )
    ...
    if not found_secrets:
        ...
        print(
            f"{Colors.BOLD}This script can only access actual GitHub secrets when run within a GitHub Actions workflow.{Colors.ENDC}"
        )
        print(
            "To use this script in a GitHub Actions workflow, add the following to your workflow YAML:"
        )
        print(
            """
    - name: List GitHub Secrets
...
def save_to_json(secrets, filename="github_secrets.json"):
    ...
    try:
        ...
        # Count found secrets
        used_found = sum(
            1 for v in secrets["used"].values() if not v.endswith("_NOT_FOUND>")
        )
        unused_found = sum(
            1 for v in secrets["unused"].values() if not v.endswith("_NOT_FOUND>")
        )
        total_found = used_found + unused_found
        total_secrets = len(secrets["used"]) + len(secrets["unused"])

        print(
            f"{Colors.BLUE}Found {total_found}/{total_secrets} secrets ({used_found} used, {unused_found} unused){Colors.ENDC}"
        )
...
def main():
    ...
    if os.environ.get("GITHUB_ACTIONS") == "true":
        ...
        print(
            f"{Colors.WARNING}Not running in GitHub Actions environment.{Colors.ENDC}"
        )
        print(
            "This script can only access actual GitHub secrets when run within a GitHub Actions workflow."
        )
        print(
            "When run locally, it will display environment variables with the same names if they exist."
        )
